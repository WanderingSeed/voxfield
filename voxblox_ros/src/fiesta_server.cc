...
FiestaServer::FiestaServer(const rclcpp::Node::SharedPtr& node)
    : FiestaServer(
...
FiestaServer::FiestaServer(
  ...
  save_esdf_map_srv_ = node_->create_service<voxblox_msgs::srv::FilePath>(
      "save_esdf_map",
      std::bind(&FiestaServer::saveEsdfMapCallback, this,
                std::placeholders::_1, std::placeholders::_2));

  save_occ_map_srv_ = node_->create_service<voxblox_msgs::srv::FilePath>(
      "save_occ_map",
      std::bind(&FiestaServer::saveOccMapCallback, this, std::placeholders::_1,
                std::placeholders::_2));

  save_all_map_srv_ = node_->create_service<voxblox_msgs::srv::FilePath>(
      "save_all_map",
      std::bind(&FiestaServer::saveAllMapCallback, this, std::placeholders::_1,
                std::placeholders::_2));
...
void FiestaServer::publishSlices() {
  ...
  pcl::toROSMsg(pointcloud, msg2);
  esdf_slice_pub_->publish(msg2);
}
...
void FiestaServer::visualizeEsdfError() {
  ...
  esdf_error_slice_pub_->publish(msg3);
}
...
void FiestaServer::publishMap(bool reset_remote_map) {
  if (!publish_esdf_map_ || !esdf_map_pub_) {
    return;
  }
    ...
    if (insert_list.size() + delete_list.size() > 0) {
      if (verbose_)
        RCLCPP_INFO(node_->get_logger(),
                    "Insert [%zu] and delete [%zu] occupied voxels.",
...
void FiestaServer::esdfMapCallback(const std::shared_ptr<voxblox_msgs::msg::Layer> layer_msg) {
  ...
  if (!success) {
    RCLCPP_ERROR_THROTTLE(node_->get_logger(), *node_->get_clock(), 10, "Got an invalid ESDF map message!");
  } else {
