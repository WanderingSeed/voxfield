...
IntensityServer::IntensityServer(const rclcpp::Node::SharedPtr& node)
  ...
  intensity_min_value = node_->declare_parameter("intensity_min_value",
                                                 intensity_min_value);
  intensity_max_value = node_->declare_parameter("intensity_max_value",
                                                 intensity_max_value);
  ...
  intensity_pointcloud_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
  ...
  intensity_image_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(
      "intensity_image", 1, 
      std::bind(&IntensityServer::intensityImageCallback, this, std::placeholders::_1));
}
...
void IntensityServer::publishPointclouds() {
  ...
  intensity_pointcloud_pub_->publish(msg);
