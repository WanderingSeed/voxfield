...
class TsdfServer {
 public:
  ...
  // ROS2 constructor accepting an rclcpp node. Implementation to be migrated.
  TsdfServer(const rclcpp::Node::SharedPtr& node);
  TsdfServer(
  ...
  // ROS2 overload: read parameters from an `rclcpp::Node`.
 ...
 protected:
  ...
  // For ROS2: stored rclcpp node (optional, used during migration).
  ...
  // ROS2 publishers/subscriptions/services/timers (optional during migration).
  ...
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr robot_model_pub_;
  ...
  // ROS2 TF broadcaster (optional during migration).
