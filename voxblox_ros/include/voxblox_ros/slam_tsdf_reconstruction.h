#ifndef VOXBLOX_ROS_SLAM_TSDF_RECONSTRUCTION_H_
#define VOXBLOX_ROS_SLAM_TSDF_RECONSTRUCTION_H_
...
class SlamTsdfReconstruction {
 public:
  ...
  virtual ~SlamTsdfReconstruction() {}

  void getServerConfigFromRosParam(const rclcpp::Node::SharedPtr& node);

  // Direct interface for processing pointcloud data
  ...
  std::shared_ptr<MeshLayer> getMeshLayerPtr() { return mesh_layer_; }
  std::shared_ptr<const MeshLayer> getMeshLayerPtr() const { return mesh_layer_; }
...
#endif  // VOXBLOX_ROS_SLAM_TSDF_RECONSTRUCTION_H_